...
typedef struct {               // STOP_START 0x1b
    ...
    BYTE Immed;
    WORD Reserved;
    BYTE Start;                // b7-b4 PowerCondition, b3-b2 reserved, b1 LOEJ, b0 Start
    BYTE Control;
} StopStartCB;
...
typedef struct
{
    ...
    BYTE Response_Data_Format; // b7,b6 Obsolete, b5 Access control coordinator, b4 hierarchical addressing support
...
typedef struct {
    ...
    unsigned PS:1;             // Parameters Savable
...
typedef union __attribute__((packed)){
    ...
    struct __attribute__((packed)){
        ...
        DWORD_VAL CmdSpecificInfo;    // depends on command on which exception occurred
        ...
        BYTE SenseKeySpecific[3];     // msb is SKSV sense-key specific valid field set=> valid SKS
